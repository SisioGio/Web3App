...
contract GreenGangPumpkins is ERC721A, Ownable {
    ...
    function buyLotteryTicket() external payable {
        ...
        require(
            checkIfUserInLotter(msg.sender) == false,
            "User has already purchased a ticket."
        );
        lottery.push(payable(msg.sender));
    }

    function checkIfUserInLotter(address userAddress)
        public
        view
        returns (bool)
    {
        for (uint256 i = 0; i < lottery.length; i++) {
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (msg.sender != owner()) {
            if (onlyWhiteListed == true) {
                ...
                require(
                    msg.value >= (PRICES[batch_index] * quantity),
                    "Not enough ether sent"
                );
            }
        }
    ...
    function mintToAddress(address _user) external payable onlyOwner {
        require(paused == false, "The contract is paused");
        require(
            1 + _numberMinted(msg.sender) <= MAX_MINTS,
            "Exceeded the limit"
        );
    ...
    function findBatchID(uint256 tokenID)
        public
        view
        virtual
        returns (uint256)
    ...
    function getAccountBalance(address _user)
        public
        view
        virtual
        returns (uint256)
    ...
    function setLotteryTicketPrice(uint256 newLotteryTicketPrice)
        public
        onlyOwner
    {
    ...
    function setBatchmaxIndex(
        uint256 batch_index,
        uint256 _newBatchMaxMintAmount
    ) public onlyOwner {
    ...
    function setBaseURI(uint256 batch_index, string memory _newBaseURI)
        public
        onlyOwner
    {
